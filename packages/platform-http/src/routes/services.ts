...
export function serviceRoutes(deps: {
  ...
  app.post('/', requireScope('project:write'), async (c) => {
    ...
    const r0 = await resolveProject(deps, op.orgSlug, op.projSlug);
    ...
    const r = await createService(
      { repos: { services: deps.services }, ids: deps.ids },
      {
  ...
  app.get('/:svcSlug', requireScope('project:read'), async (c) => {
    ...
    const s = await deps.services.findBySlug(r0.value.project.id, svcSlug);
    if (!isOk(s) || !s.value)
