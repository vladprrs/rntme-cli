...
export async function resetSchema(pool: Pool): Promise<void> {
  await pool.query(
    `TRUNCATE TABLE deployment_log_line, deployment, deploy_target, audit_log, event_outbox, project_version, project, api_token, membership_mirror, workos_event_log, account, organization RESTART IDENTITY CASCADE;`,
  );
}
