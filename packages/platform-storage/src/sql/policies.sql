...
CREATE POLICY tenant_isolation_deploy_target ON deploy_target USING (org_id = current_setting('app.org_id', true)::uuid) WITH CHECK (org_id = current_setting('app.org_id', true)::uuid);
CREATE POLICY tenant_insert_deploy_target ON deploy_target FOR INSERT WITH CHECK (org_id = current_setting('app.org_id', true)::uuid);
CREATE POLICY tenant_isolation_deployment ON deployment USING (org_id = current_setting('app.org_id', true)::uuid) WITH CHECK (org_id = current_setting('app.org_id', true)::uuid);
CREATE POLICY tenant_insert_deployment ON deployment FOR INSERT WITH CHECK (org_id = current_setting('app.org_id', true)::uuid);
CREATE POLICY tenant_isolation_deployment_log_line ON deployment_log_line USING (org_id = current_setting('app.org_id', true)::uuid) WITH CHECK (org_id = current_setting('app.org_id', true)::uuid);
CREATE POLICY tenant_insert_deployment_log_line ON deployment_log_line FOR INSERT WITH CHECK (org_id = current_setting('app.org_id', true)::uuid);
